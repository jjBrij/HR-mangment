...
import { format, startOfMonth, endOfMonth, eachDayOfInterval, isSameMonth, isSameDay, addMonths, subMonths, differenceInDays, isValid, isBefore, startOfToday } from 'date-fns';
...
const EmployeeDashboard = () => {
  ...
  // Get current logged-in user (for demo, using first employee or stored user)
  useEffect(() => {
    loadEmployeeData();
  }, []);
  ...
  const loadEmployeeData = () => {
    // For demo, get the first employee or check session storage for logged-in user
    let employee = null;
  ...
  const loadDailyTargets = () => {
    // Load existing daily targets from localStorage
    ...
    // Filter targets for current employee and selected month
    ...
    daysInMonth.forEach(date => {
      ...
      if (!existingDates.includes(dateStr) && isBefore(date, today) === false) {
        // Only create future dates, not past dates
        missingTargets.push({
    ...
    // Combine existing and missing targets
  ...
  const calculateStatistics = (targets) => {
    ...
    // Calculate average performance based on completion rate
    const averagePerformance = completionRate;
  ...
  const applyFilters = () => {
    let filtered = [...dailyTargets];
    
    // Filter only future or today's dates (not past dates for editing)
    const today = startOfToday();
  ...
  const saveEdit = async (id) => {
    ...
    const updatedTarget = {
      ...
      status: editFormData.completedAmount >= editFormData.targetAmount ? 'Completed' : 'In Progress',
  ...
  const getStatusBadge = (targetAmount, completedAmount) => {
    if (completedAmount >= targetAmount && targetAmount > 0) {
      return <span className="inline-flex items-center px-2 py-1 text-xs font-medium rounded-full bg-green-100 text-green-700"><FiCheckCircle className="mr-1" size={12} />Completed</span>;
    } else if (completedAmount > 0) {
      return <span className="inline-flex items-center px-2 py-1 text-xs font-medium rounded-full bg-blue-100 text-blue-700"><FiActivity className="mr-1" size={12} />In Progress</span>;
    } else {
    ...
    <><div className="animate-fade-in pb-8">
          ...
          <div className="bg-gradient-to-r from-indigo-600 to-indigo-800 rounded-2xl p-6 mb-6 text-white">
              <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                  ...
                  <div className="text-center md:text-right">
                      ...
                      <p className="text-xl font-mono">{currentEmployee.employeeId}</p>
                      <p className="text-sm opacity-80 mt-2">Joining Date</p>
                      <p>{currentEmployee.joinDate}</p>
                  </div>
              </div>
          </div>
              ...
              <div className="stat-card">
                  <div className="flex items-center justify-between">
                      <div>
                          ...
                          <p className="text-2xl font-bold text-purple-600 mt-1">{statistics.completionRate}%</p>
                      </div>
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
              <div className="p-6 border-b border-gray-200">
                  ...
                  <p className="text-sm text-gray-500 mt-1">Set and track your daily performance targets</p>
                  <p className="text-xs text-amber-600 mt-1">⚠️ You can only edit future dates. Past dates are locked.</p>
              </div>
                  ...
                  <table className="w-full">
                      <thead className="bg-gray-50 border-b border-gray-200">
                          <tr>
                              ...
                              <th className="text-right px-4 py-3 text-xs font-semibold text-gray-500 uppercase">Target Amount</th>
                              <th className="text-right px-4 py-3 text-xs font-semibold text-gray-500 uppercase">Completed Amount</th>
                      ...
                      <tbody className="divide-y divide-gray-100">
                          {filteredTargets.length > 0 ? (
                              filteredTargets.map((target) => {
                                  ...
                                  const progress = target.targetAmount > 0 ? (target.completedAmount / target.targetAmount) * 100 : 0;

                                  return (
                                      <tr key={target.id} className="hover:bg-gray-50 transition">
                                          <td className="px-4 py-3">
                                              <span className="text-sm font-medium text-gray-800">
                                                  {format(new Date(target.date), 'dd MMM yyyy')}
                                              </span>
                                              {isPastDate && (
                                          ...
                                          <td className="px-4 py-3 text-right">
                                              {editingTarget === target.id && !isPastDate ? (
                                                  <input
                                                      type="number"
                                                      name="targetAmount"
                                                  ...
                                                  <span className="text-sm font-semibold text-gray-800">{formatCurrency(target.targetAmount)}</span>
                                              )}
                                          </td>
                                          <td className="px-4 py-3 text-right">
                                              {editingTarget === target.id && !isPastDate ? (
                                                  ...
                                                  <span className="text-sm text-green-600">{formatCurrency(target.completedAmount)}</span>
                                              )}
                                          </td>
                                          <td className="px-4 py-3">
                                              <div className="flex items-center gap-2">
                                                  ...
                                                  <span className="text-xs text-gray-600 w-12">{progress.toFixed(0)}%</span>
                                              </div>
                                          </td>
                                          <td className="px-4 py-3 text-center">
                                              {getStatusBadge(target.targetAmount, target.completedAmount)}
                                          </td>
                                          <td className="px-4 py-3 text-center">
                  ...
                  <tr>
                    <td colSpan="6" className="text-center py-12">
                      <div className="text-center">
                        ...
                        <p className="text-sm text-gray-400 mt-1">New targets will be auto-generated for future dates</p>
                      </div>
                    </td>
                  </tr>
                )}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <div className="w-full bg-gray-200 rounded-full h-3">
                    <div 
                      className={`${getProgressColor(parseFloat(statistics.completionRate))} h-3 rounded-full transition-all duration-500`}
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-blue-700 mt-1">
                • You can set daily targets for future dates only. Past dates are locked for editing.<br />
                • Your completed targets will automatically update your performance score.<br />
                • The performance score is calculated as: (Completed Amount / Target Amount) × 100<br />
                • Your progress is shown in the Performance Management dashboard.
              </p>
            </div>
          </div>
        </div>
